...
export default function FormChat() {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        // Get first question
        const nq = await axios.post(
          `${API_BASE}/sessions/${res.data.sessionId}/next-question`,
          {},
        );
        ...
        if (nq.data.nextQuestion) {
          setMessages([
            {
              type: "bot",
              text: `Xin chào! Tôi sẽ giúp bạn điền biểu mẫu "${
                formRes.data.title || formId
  ...
  const submitAnswer = async () => {
    ...
    const newMessages = [
      ...messages,
      { type: "user" as const, text: userAnswer },
    ];
    setMessages(newMessages);

    try {
      const nq = await axios.post(
        `${API_BASE}/sessions/${sessionId}/next-question`,
        {
      ...
      if (validation && validation.needsConfirmation) {
        ...
        const confirmMessages = [
          ...newMessages,
          {
            type: "warning",
            text: `⚠️ ${
              validation.message || "Vui lòng kiểm tra lại câu trả lời của bạn."
            }`,
          },
        ];
  ...
  const finish = async () => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE}/sessions/${sessionId}/fill`,
        { answers: {} },
        { responseType: "blob" },
      );
        ...
        <div style={styles.messagesContainer}>
                ...
                onMouseEnter={(e) => {
                  if (!loading) {
                    ...
                    e.currentTarget.style.boxShadow =
                      "0 6px 16px rgba(17, 153, 142, 0.5)";
                  }
                }}
                onMouseLeave={(e) => {
          ...
          {pdfUrl && (
            <div style={{ textAlign: "center" as const }}>
              ...
              >
                Vị trí các field không chính xác?
              </div>
              <button
                style={{
                  ...styles.downloadLink,
                ...
                onClick={() => {
                  router.push(
                    `/admin/bbox-editor?formId=${encodeURIComponent(formId)}`,
                  );
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.transform = "translateY(-2px)";
